...
export default class SimulationContainer extends Component {
  ...
  render() {
    ...
    const announcementsList =
      announcements.length > 0 ? (
        announcements.map((a, i) => (
          <div key={`${a.id}${a.time}`}>{`${i + 1} - ${a.type} (#${
            a.id
          })`}</div>
        ))
      ) : (
    ...
    const actionsList = simulation.actions.map((action) => {
      let destination = '';
      ...
      return (
        <div
          ...
          className={`bp5-minimal ${
            action.time < currentMinutes ? styles.done : ''
          }`}
          key={`${action.event.id}_${action.time}_${action.do}`}
        >
          {`(#${action.event.id}) `}
          {_date.minutesToHm(action.time)} → {destination}
          <Button
            className={`bp5-minimal ${
              action.time < currentMinutes ? styles.done : ''
            }`}
    ...
    return (
      <div>
        ...
        <div className="bp5-callout" style={{ fontSize: '80%' }}>
          Here you can control the system simulation somewhat. Be aware it will
          change statuses of events. After you enter the password, you can use
          all the actions.
        </div>
        <p />
        <div>
        ...
        <div>
          ...
          <div className="bp5-callout">
            The list below reads from left to right, and new line starts from
            the left ⥅.
          </div>
          <p />
          <div
